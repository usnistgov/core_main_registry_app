...
admin_urls = [
    ...
    re_path(r'^data', common_views.ViewData.as_view(administration=True,
                                                    template='core_main_registry_app/admin/data/view_data.html'),
            name='core_main_app_data_detail'),
    re_path(r'^templates$', registry_admin_views.manage_templates, name='core_main_registry_app_templates'),
    re_path(r'^template/upload/(?P<version_manager_id>\w+)', admin_views.upload_template_version,
            name='core_main_app_upload_template_version'),
    re_path(r'^template/versions/(?P<version_manager_id>\w+)', admin_views.manage_template_versions,
    ...
    re_path(r'^template/resolve-dependencies', admin_ajax.resolve_dependencies,
            name='core_main_app_resolve_dependencies'),
    re_path(r'^xslt/(?P<pk>[\w-]+)/edit/$', admin_ajax.EditXSLTView.as_view(),
            name='core_main_app_edit_xslt'),
    re_path(r'^xslt$', admin_views.XSLTView.as_view(), name='core_main_app_xslt'),
    re_path(r'^xslt/upload$', admin_views.UploadXSLTView.as_view(), name='core_main_app_upload_xslt'),
    re_path(r'^xslt/(?P<pk>[\w-]+)/delete/$',
            admin_ajax.DeleteXSLTView.as_view(),
            name='core_main_app_delete_xslt'),
    re_path(r'^edit-rights/(?P<workspace_id>\w+)$',
    ...
    re_path(r'^custom-registry$', staff_member_required(registry_admin_views.CustomRegistry.as_view()),
            name='core_main_registry_app_custom_registry'),
    re_path(r'^template/(?P<template_id>\w+)/custom-resource',
